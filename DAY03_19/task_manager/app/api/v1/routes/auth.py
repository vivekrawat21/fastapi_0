...
async def login(
    ...
    refresh_token = create_referesh_token(data={"sub": user.email})
...
async def register(
    ...
    """Register a new user"""
    user = await user_service.create_user_auth(register_request.name, register_request.email, register_request.password)
    return user
